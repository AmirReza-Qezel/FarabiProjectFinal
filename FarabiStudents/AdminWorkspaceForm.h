...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->update_btn->Text = L"Update Existing Student Or Info";
			...
			this->remove_btn->Text = L"Remove Existing Student Or Info";
...
private: System::Void showallrbtn_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
	...
	conn.Open();
	String^ query = "SELECT S.*,(SELECT SUM(C.Chemer) / SUM(C.Units) FROM dbo.CoursesScore AS C WHERE C.StudentID = S.StudentID) AS TotalScore FROM dbo.Students AS S ORDER BY (SELECT SUM(C.Chemer) / SUM(C.Units)FROM dbo.CoursesScore AS C WHERE C.StudentID = S.StudentID) Desc;";
...
private: System::Void showcerbtn_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
	...
	SqlConnection conn(constr);
	conn.Open();
	String^ query = "SELECT S.*,(SELECT SUM(C.Chemer) / SUM(C.Units) FROM dbo.CoursesScore AS C WHERE C.StudentID = S.StudentID) AS TotalScore FROM dbo.Students AS S WHERE S.Major = 'CE' ORDER BY (SELECT SUM(C.Chemer) / SUM(C.Units)FROM dbo.CoursesScore AS C WHERE C.StudentID = S.StudentID) Desc";
	SqlCommand^ command = gcnew SqlCommand(query, % conn);
	DataTable dtbl;
	SqlDataReader^ myReader;
	SqlDataAdapter^ sda = gcnew SqlDataAdapter();
...
private: System::Void showierbtn_CheckedChanged(System::Object^ sender, System::EventArgs^ e) {
	...
	SqlConnection conn(constr);
	conn.Open();
	String^ query = "SELECT S.*,(SELECT SUM(C.Chemer) / SUM(C.Units) FROM dbo.CoursesScore AS C WHERE C.StudentID = S.StudentID) AS TotalScore FROM dbo.Students AS S WHERE S.Major = 'IE' ORDER BY (SELECT SUM(C.Chemer) / SUM(C.Units)FROM dbo.CoursesScore AS C WHERE C.StudentID = S.StudentID) Desc";
	SqlCommand^ command = gcnew SqlCommand(query, % conn);
	DataTable dtbl;
	SqlDataReader^ myReader;
	SqlDataAdapter^ sda = gcnew SqlDataAdapter();
